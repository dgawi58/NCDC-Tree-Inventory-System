the following script as build_repo_bundle.sh on a Linux/macOS machine (bash). It will:

    Create the NCDC_Tree_Inventory folder
    Populate every file from the bundle (structured)
    Add the NCDC logo and sample import files
    Add LICENSE (AT‑SCL 2026) and README
    Create repo.zip in the current directory
    Optionally run create_repo.sh to initialize and push (if you confirm)

Instructions:

    Save below content to build_repo_bundle.sh
    Make executable: chmod +x build_repo_bundle.sh
    Run: ./build_repo_bundle.sh
    After it finishes, run ./create_repo.sh (it will be created inside the repo) to create & push the GitHub repo.

build_repo_bundle.sh:
#!/usr/bin/env bash
set -e
